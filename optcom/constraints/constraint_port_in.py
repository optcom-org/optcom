...
# Exceptions
class PortInError(ConstraintError):
    pass

class PortInWarning(ConstraintWarning):
    pass


class ConstraintPortIn(AbstractConstraint):
    """Port need to be allow to accept an input."""
    ...
    def is_respected(self, comp: AbstractComponent, comp_port_id: int,
        ...
        flag = ngbr[ngbr_port_id].is_type_in()
        if (not flag):
            warning_message: str = ("Port {} of component {} does not "
                "accept input, field will be ignored."
                .format(ngbr_port_id, ngbr.name))
            warnings.warn(warning_message, PortInWarning)
